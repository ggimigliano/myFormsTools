...
function check_files_extensions_on_path($path, $ext)
{
	...
	else
	{
		...
		{
			check_files_extensions_on_path(trim((string) $path, '/') . "/" . $file, $ext);
		}
	}
}
...
function rcopy($source, $destination, $is_rec = false)
{
	if (is_dir($source))
	{
		if ($is_rec === false)
		{
			...
			$destination = rtrim((string) $destination, '/') . DIRECTORY_SEPARATOR . $pinfo['basename'];
		}
		...
		foreach ($files as $file)
		{
			if ($file != "." && $file != "..")
			{
				rcopy($source . DIRECTORY_SEPARATOR . $file, rtrim((string) $destination, '/') . DIRECTORY_SEPARATOR . $file, true);
			}
		}
	}
	else
	{
		if (file_exists($source))
		{
			...
			{
				$pinfo = pathinfo($source);
				$dest2 = rtrim((string) $destination, '/') . DIRECTORY_SEPARATOR . $pinfo['basename'];
			}
			else
			{
				$dest2 = $destination;
...
function rrename($source, $destination, $is_rec = false)
{
	if (is_dir($source))
	{
		...
		foreach ($files as $file)
		{
			if ($file != "." && $file != "..")
			{
				rrename($source . DIRECTORY_SEPARATOR . $file, rtrim((string) $destination, '/') . DIRECTORY_SEPARATOR . $file, true);
			}
		}
	}
	else
