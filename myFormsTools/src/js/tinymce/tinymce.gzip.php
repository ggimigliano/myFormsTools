...
//BODY TUTTO COMMENTATO PERCHE' RISIEDE IN mytinmce.gzip.php
...
class TinyMCE_Compressor {
	...
	public function handleRequest() {
		...
		If(isset($_SERVER['HTTP_IF_NONE_MATCH']) && 
		        $this->settings['disk_cache'] &&  file_exists($cacheFile) &&
		        $_SERVER['HTTP_IF_NONE_MATCH'] == "_".sha1($cacheFile)."_".filemtime($cacheFile)) 
		          {
		...
		if ($this->settings['disk_cache'] && file_exists($cacheFile)) {
		    header("Etag: _".sha1($cacheFile)."_".filemtime($cacheFile));
		    readfile($cacheFile);
			return;
		}
...
//				$buffer .= "\n//-FILE-$tinymceDir/$file (". strlen($fileContents) . " bytes)\n";
	...
	public static function renderTag($tagSettings, $return = false) {
		...
		$scriptTag = '<script src="' .\Gimi\myFormsTools\PckmyUtils\PHP8::htmlspecialchars($scriptSrc) . '"></script>';
