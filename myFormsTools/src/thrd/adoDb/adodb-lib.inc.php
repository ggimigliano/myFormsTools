...
function _adodb_replace($zthis, $table, $fieldArray, $keyCol, $autoQuote, $has_autoinc)
{
	...
	$iCols = ltrim((string) $iCols, ',');
	$iVals = ltrim((string) $iVals, ',');
			...
			$size=0, $selectAttr='',$compareFields0=true)
{
	...
	while(!$zthis->EOF) {
		...
		if ($hasvalue) {
			...
			$zval2 = trim((string) $zval2);
			$value = 'value="' .\Gimi\myFormsTools\PckmyUtils\PHP8::htmlspecialchars($zval2) . '"';
		}
	...
	while(!$zthis->EOF) {
		...
		if ($hasvalue) {
			if ($ADODB_FETCH_MODE == ADODB_FETCH_ASSOC) {
				...
				$zval2 = $zthis->fields[1];
				if ($hasgroup) {
					$group = trim((string) $zthis->fields[2]);
				}
			}
			$zval2 = trim((string) $zval2);
			$value = "value='".\Gimi\myFormsTools\PckmyUtils\PHP8::htmlspecialchars($zval2)."'";
		...
		if ($optgroup != $group) {
			...
			$s .="\n<optgroup label='".\Gimi\myFormsTools\PckmyUtils\PHP8::htmlspecialchars($group) ."'>";
		}
...
function _adodb_getmenu_option($defstr, $compare, $value, $display)
{
	...
	return "\n<option $value$selected>" . \Gimi\myFormsTools\PckmyUtils\PHP8::htmlspecialchars($display) . '</option>';
}
...
*
* @return  void A handle to the executed query
*/
function _adodb_debug_execute($zthis, $sql, $inputarr)
{
	...
	if ($inBrowser) {
		if ($ss) {
			...
			$ss = sprintf('<code class="adodb-debug">%s</code>',\Gimi\myFormsTools\PckmyUtils\PHP8::htmlspecialchars($ss));
		}
		if ($zthis->debug === -1) {
			$outString = "<br class='adodb-debug'>(%s):  %s &nbsp; %s<br class='adodb-debug'>";
			ADOConnection::outp(sprintf($outString, $myDatabaseType, \Gimi\myFormsTools\PckmyUtils\PHP8::htmlspecialchars($sqlTxt), $ss), false);
	...
	if ($zthis->databaseType == 'mssql') {
		...
		if ($emsg = $zthis->ErrorMsg()) {
			if ($err = $zthis->ErrorNo()) {
				if ($zthis->debug === -99) {
					ADOConnection::outp("<hr>\n($myDatabaseType): " . \Gimi\myFormsTools\PckmyUtils\PHP8::htmlspecialchars($sqlTxt) . " &nbsp; $ss\n<hr>\n", false);
				}
		...
		if (isset($arr['args'])) {
			foreach ($arr['args'] as $v) {
				if (is_null($v)) {
					...
					$str =\Gimi\myFormsTools\PckmyUtils\PHP8::htmlspecialchars($v);
