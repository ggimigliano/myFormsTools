				...
				foreach($inputarr as $arr) {
					...
					if (isset($sqlarr[$i])) {
						...
						if ($i+1 != sizeof($sqlarr)) {
							$this->outp_throw( "Input Array does not match ?: ".\Gimi\myFormsTools\PckmyUtils\PHP8::htmlspecialchars($sql),'Execute');
						}
	 ...
	 public function CacheExecute($secs2cache,$sql=false,$inputarr=false) {
		...
		if (!$rs) {
			...
			if ($this->debug){
				...
				if ($inBrowser) {
					$s = '<i>'.\Gimi\myFormsTools\PckmyUtils\PHP8::htmlspecialchars($s).'</i>';
				}
...
*/
	 ...
	 * and/or last one of the recordset. Added by Ivà¡n Oliva to provide recordset pagination.
	 *
...
class ADORecordSet implements IteratorAggregate {
	...
	public $_currentPage = -1;	/** Added by Ivà¡n Oliva to implement recordset pagination */
	public $_atFirstPage = false;	/** Added by Ivà¡n Oliva to implement recordset pagination */
	public $_atLastPage = false;	/** Added by Ivà¡n Oliva to implement recordset pagination */
	 ...
	 * @return string HTML
	 */
	 public function getMenuGrouped($name, $defstr = '', $blank1stItem = true, $multiple = false,
							$size = 0, $selectAttr = '', $compareFirstCol = true)
	{
		global $ADODB_INCLUDED_LIB;
		if (empty($ADODB_INCLUDED_LIB)) {
	...
	class ADORecordSet_array extends ADORecordSet
	{
		 ...
		 */
		 public function FetchField($fieldOffset = -1) {
			...
			if(isset($this->_types[$fieldOffset])) $o->name = $this->_colnames[$fieldOffset];
			if(isset($this->_types[$fieldOffset])) $o->type = $this->_types[$fieldOffset];
		 ...
		 */
		 public function _seek($row) {
			if (sizeof($this->_array) && 0 <= $row && $row < $this->_numOfRows) {
				...
				if(isset($this->_array[$row])) $this->fields = $this->_array[$row];
				return true;
			}
			return false;
		}
		 ...
		 */
		 public function _fetch() {
			...
			if(isset($this->_array[$pos])) $this->fields = $this->_array[$pos];
			return true;
		}
