	 ...
	 public function _query($sql, $inputarr)
	{
		...
		else if (is_string($sql) && is_array($inputarr))
		{
			...
			*/
			...
			{
				$this->outp_throw(
					"SQL Statement failed on preparation: " . \Gimi\myFormsTools\PckmyUtils\PHP8::htmlspecialchars($sql) . "'",
					'Execute'
				);
				return false;
			}
			...
			*/
			...
			if ($nparams  != $inputArrayCount)
			{
				...
				$this->outp_throw(
					...
					" params, does not match query: '" . \Gimi\myFormsTools\PckmyUtils\PHP8::htmlspecialchars($sql) . "'",
					'Execute'
				);
				return false;
			}
