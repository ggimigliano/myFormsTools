...
/*
 * - Rimosso namespaces
 * - costante Ë variabile statica
 * - upper preserva i link
 * - converter traslittera caratteri
 *  
 *   protected function converter(&$text)
    {
        ...
        $sost=array('da'=>array('î',       'ì',       'í',      '`',     'ë',   'í',  'ñ',   'Ö',
            '&rdquo;','&ldquo;','&rsquo;','&rsquo;', '&lsquo;',   '&hellip;' ,'&amp;','&euro;','&ndash;',"ﬂ"),
            'ad'=>array('"',      '"',        "'",      "'",      "'",   "'",  '-',   '...',
                '"',       '"',        "'",      "'",      "'",         '...' ,     '&',     'Ä',     '-','SS'
            )
        );
        
        foreach ($sost['da'] as &$chr) {
            if(is_callable('iconv')) $chr=iconv('iso-8859-1', self::$ENCODING.'//TRANSLIT//IGNORE',$chr);
            elseif(is_callable('mb_convert_encoding'))  $chr=mb_convert_encoding($chr, self::$ENCODING,'iso-8859-1');
        }
        
        
        $text=str_replace($sost['da'],$sost['ad'],$text);
    ...
    protected function toupper($str)
    {
        ...
        foreach ($chunks as $i => $chunk) 
            if ( $chunk[0] != '<' &&
                ...
                !preg_match('#^(mailto:)?(?=[A-Z0-9][A-Z0-9@._%+-]{5,253}+$)[A-Z0-9._%+-]{1,64}+@(?:(?=[A-Z0-9-]{1,63}+\.)[A-Z0-9]++(?:-[A-Z0-9]++)*+\.){1,8}+[A-Z]{2,63}+$#',trim((string) $chunk)) &&
                !preg_match('£^(ftp|http|https):\/\/(\w+:{0,1}\w*@)?(\S+)(:[0-9]+)?(\/|\/([\w#!:.?+=&%@!\-\/]))?£',trim((string) $chunk)  ) 
    ...
    {    
         $LATIN1_UC_CHARS="¿¡¬√ƒ≈∆«»… ÀÃÕŒœ–—“”‘’÷ÿŸ⁄€‹›";
         $LATIN1_LC_CHARS="‡·‚„‰ÂÊÁËÈÍÎÏÌÓÔÒÚÛÙıˆ¯˘˙˚¸˝";
         $str = html_entity_decode($str, ENT_COMPAT, self::$ENCODING);
         $str = strtoupper(strtr($str, $LATIN1_LC_CHARS, $LATIN1_UC_CHARS));
         preg_match_all('@&[A-Z]+;@',$str,$m);
         if($m) foreach ($m[0] as $k) $str=str_replace($k,strtolower($k),$str);
         return $str;
    }
            ...
            foreach ($matches[0] as $m) {
                if ($m[0][0] == '<' && $m[0][1] == '/') {
                    ...
                    if ($level < 0) {
                        ...
                        $body = preg_replace('/((^|\n)>*)/', '\\1> ', trim((string) $body));
                        $body = '<pre>' .\Gimi\myFormsTools\PckmyUtils\PHP8::htmlspecialchars($body) . '</pre>';
                        // Re-set text width
