...
abstract class PHP8
{
	...
	public  static function strftime (string $format, $timestamp = null, ?string $locale = null) : string {
		if (!($timestamp instanceof DateTimeInterface)) {
			try {
				if(intval($timestamp)==$timestamp)   $timestamp = (new DateTime())->setTimestamp($timestamp);
												else $timestamp = new DateTime($timestamp);
			} catch (Exception $e) {
	...
	public static function htmlspecialchars($string,$second=null,$charset=null){
		return htmlspecialchars((string) $string,$second?$second:ENT_QUOTES | ENT_SUBSTITUTE | ENT_HTML401,$charset);
	}
}
