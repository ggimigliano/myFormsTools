...
function addOptionsFromXml(x,element){
for(var i=0;i<x.length;i++) 
			...
			if(x[i].attributes) for (var j=0;j<x[i].attributes.length;j++)  opt.setAttribute(x[i].attributes[j].nodeName, x[i].attributes[j].nodeValue);
...
try{
	 if(element.childNodes)  while (element.firstChild) {
...
var x=x.childNodes;
for(var i=0;i<x.length;i++) {
  if(x[i].nodeName.trim()=='optgroup') 
			{
			
			var opt=document.createElement('optgroup');
			if(x[i].attributes) for (var j=0;j<x[i].attributes.length;j++)  opt.setAttribute(x[i].attributes[j].nodeName, x[i].attributes[j].nodeValue);
			if(x[i].childNodes) addOptionsFromXml(x[i].childNodes,opt )
			element.add(opt,element.options[null]);
			}
  if(x[i].nodeName.trim()=='option') 
			{
			var opt=document.createElement('option');
			if(x[i].childNodes[0] && x[i].childNodes[0].nodeValue)
			 				    opt.text=''+x[i].childNodes[0].nodeValue;
